...
async def get_input_tips(keyword: str, lat: float, lon: float):
    ...
    params = {"key": AMAP_KEY, "keywords": keyword, "location": f"{gcj_lon},{gcj_lat}", "datatype": "all"}
    try:
...
async def upload_memory(request_data: UploadRequest, request: Request):
    ...
    if request_data.exact_name and request_data.exact_location:
        ...
        else: raise HTTPException(status_code=400, detail="Must provide text or image")
        try:
            response = await client.chat.completions.create(model="qwen-vl-max" if request_data.image_base64 else "qwen-max", messages=messages, response_format={"type": "json_object"})
        ...
        try:
            async with httpx.AsyncClient() as http_client: search_res = (await http_client.get(amap_url, params=params)).json()
